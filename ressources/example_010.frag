...
vec3 noiseCircle( vec2 _st, vec2 center, float _radius, float _thickness, float _noiseRoughness, float _noiseStrength, float _timeOffset )
{
	vec2 diff = _st - center;
...
void main(void)
{
	...
	vec2 center = vec2( mouse_x / resolution.x, 1 - (mouse_y / resolution.y) );

	vec3 c1 = noiseCircle( st, center, (0.5 / 8) + 0.1, (glowness / 100) + 0.001, phase / 10, height / 80, 0.0 ) * lightColor;
    lightColor = vec3(0.2, 0.6, 1.0);
	vec3 c2 = noiseCircle( st, center, (0.5 / 8) + 0.1, (glowness / 100) + 0.001, phase / 10, height / 70, 1.234 ) * lightColor;
    lightColor = vec3(1.0, 0.2, 1.0);
	vec3 c3 = noiseCircle( st, center, (0.5 / 8) + 0.1, (glowness / 100) + 0.001, phase / 10, height / 60, 5.678 ) * lightColor;
